...
export function CanvasNodeItem({ node, isSelected, onMouseDown, onClick }: CanvasNodeItemProps) {
    ...
    return (
      ...
      >
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
          <JournalingEditor variant="canvas" className="border-none" />
        </div>
      </div>
    );
  }
  ...
  return (
    <article
      ...
      className={cn(
        "absolute w-[16rem] cursor-pointer rounded-xl border bg-white p-4 text-left shadow-sm  hover:shadow-md hover:border-slate-300",
        node.highlight ? "border-indigo-200 ring-2 ring-indigo-50" : "border-slate-200",
